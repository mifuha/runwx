...
event-to-revision reference. Local-only successes remain hidden. A separate guarded
writer is now tested locally, with native BigQuery validation pending; the
in-memory session is not that writer.
