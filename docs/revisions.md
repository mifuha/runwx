...
event-to-revision reference. Local-only successes remain hidden. The separate
guarded writer has also passed native update, stale-request and rollback checks;
the in-memory session remains only the local contract.
